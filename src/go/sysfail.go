...
static int filterStdOutStdErr(void* ctx, const greg_t* regs) {
...
static void set_null_ptr(sysfail_syscall_outcome_t* outcome) {
...
static void set_eligible_func_no_stdout_stderr(sysfail_outcome_t* outcome) {
	if(outcome) outcome->eligible = filterStdOutStdErr;
}
...
type SelectorT func(ctx unsafe.Pointer, tid C.sysfail_tid_t) int

type EligibleFuncT func(ctx unsafe.Pointer, regs *C.greg_t) int

// SyscallNumber returns the syscall number.
func SyscallNumber(regs *C.greg_t) int {
	return int(C.sysfail_syscall((*C.greg_t)(regs)))
}

// SyscallArg returns the syscall argument.
func SyscallArg(regs *C.greg_t, num int) C.greg_t {
	return C.greg_t(C.sysfail_syscall_arg((*C.greg_t)(regs), C.int(num)))
}
...
type EligibilityFuncType int

const (
	EligibleFuncNone EligibilityFuncType = iota
	EligibleFuncCustom
	EligibleFuncStdOutStdErr
)

type Eligibility struct {
	// Function to determine if the thread is eligible for failure injection.
	Eligible EligibleFuncT
	// Type of eligibility function
	Type EligibilityFuncType
}
...
func StartSession(plan *Plan) (*Session, error) {
	...
	if len(plan.SyscallOutcomes) > 0 {
		...
		for i, outcome := range plan.SyscallOutcomes {
			...
			case EligibleFuncCustom:
				return nil, ErrCustomEligibilityNotSupported
			case EligibleFuncStdOutStdErr:
				C.set_eligible_func_no_stdout_stderr(&curr.outcome)
			default:
